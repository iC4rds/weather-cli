...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      console.log(`Weather in ${geoData.name}, ${geoData.state || ''} ${geoData.country}:`);

      const weatherResponse = await axios.get<WeatherData>(
        `${WEATHER_API_URL}?lat=${geoData.lat}&lon=${geoData.lon}&appid=${API_KEY}&units=${options.units}`
      );
      const weatherData = weatherResponse.data;

      console.log(`----------------------------------------------`);
      console.log(weatherData.weather[0].description);
      console.log(`- Temperature: ${weatherData.main.temp}°${options.units === 'metric' ? 'C' : 'F'}`);
      console.log(`- Feels Like: ${weatherData.main.feels_like}°${options.units === 'metric' ? 'C' : 'F'}`);
